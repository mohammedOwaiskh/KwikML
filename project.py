...
class kwikml:
    ...
    def set_classifier_properties(self):
        ...
        if self.type == "Regression":
            self.chosen_classifier = st.selectbox("Select Regression Algorithm", ('Random Forest', 'Linear Regression', 'K-Nearest Neighbor')) 
            if self.chosen_classifier == 'Random Forest': 
                self.n_trees = st.slider('number of trees', 1, 1000, 1)
   
        elif self.type == "Classification":
            self.chosen_classifier = st.selectbox("Select Classifier", ('Logistic Regression', 'Naive Bayes', 'K-Nearest Neighbor')) 
    ...
    def get_metrics(self):
        ...
        if self.type == 'Regression':
            ...
            ' -- MSE Test Score: ' + str(round(self.error_metrics['MSE_test'], 3)))
    ...
    def plot_result(self):
        ...
        if self.result.Actual is not None:
            s2 = figure(plot_width=800, plot_height=500, background_fill_color="#5f27cd")
    ...
    def file_selector(self):
        dataset_import = st.sidebar.selectbox("Where do you want to import data set from?",['Select Import type','Pre-Loaded','Upload'])
        if dataset_import == 'Pre-Loaded':
            dataset = st.sidebar.selectbox("Select Dataset",['Select a Dataset','Titanic Data','Place, Area & Price','Diabetes Dataset'])
    ...
    def print_table(self):
        if len(self.result) > 0:
            ...
            st.dataframe(result.sort_values(by='Actual',ascending=False).style.highlight_max(color = "#FF4B4B", axis=0))
    
    def set_features(self):
        self.features = st.multiselect('Select all the variables that go into the model(features + target)', self.data.columns )
    ...
    try:
        ...
        if controller.data is not None:
            ...
            with st.sidebar.expander('Data Visualization'):
                type_of_plot = st.selectbox("Select Type of Plot",["","Area","Bar","Line","Pie"])
            ...
            try:    
                if type_of_plot == 'Bar':
                    ...
                    if want_query:
                        ...
                        value = st.selectbox("Select the value from {}".format(query),all_values)
                ...
                st.warning('Incompatible column selected. Try selecting other columns or To know more about the data click on \'Display Data\'.')
